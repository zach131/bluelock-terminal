...
import { awardXp } from "@/lib/globalXp"; // UPDATED: Import awardXp
import { XP_VALUES } from "@/lib/constants"; // ADDED: Import constants
...
interface WorkoutSession {
  id: string;
  date: string;
  type: "push" | "pull" | "legs" | "cardio" | "plyo" | "other";
  exercises: Exercise[];
  totalVolume: number;
}
interface PlyometricLog {
  date: string;
  verticalJump: number;
  broadJump?: number;
}
...
export default function BiologicalLedger() {
  ...
  const [workoutType, setWorkoutType] =
    useState<WorkoutSession["type"]>("push");
    ...
    setCurrentWorkout((prev) =>
      prev
        ? {
            ...
            exercises: [
              ...prev.exercises,
              { name: exerciseName.trim(), sets: [...sets], isPR },
            ],
    ...
    if (isPR)
      setData((prev) => ({
        ...
        personalRecords: {
          ...
          [exerciseName.toLowerCase()]: bestWeight,
        },
      }));
    setExerciseName("");
    setSets([]);
  ...
  const finishWorkout = () => {
    ...
    setData((prev) => {
      const newWorkouts = [...prev.workouts, currentWorkout];
      return {
        ...prev,
        workouts: newWorkouts,
        streak: calculateStreak(newWorkouts),
        lastWorkoutDate: currentWorkout.date,
        totalWorkouts: prev.totalWorkouts + 1,
      };
    });

    // UPDATED: Use awardXp for BODY
    awardXp(XP_VALUES.WORKOUT_LOGGED, "BODY");
    ...
    setData((prev) => ({
      ...
      plyometrics: [
        ...prev.plyometrics,
        {
          date: getToday(),
          verticalJump: vj,
          broadJump: broadJump ? parseFloat(broadJump) : undefined,
        },
      ],
    }));
    setVerticalJump("");
  ...
  const latestVJ =
    data.plyometrics.length > 0
      ? data.plyometrics[data.plyometrics.length - 1].verticalJump
      : null;
  const bestVJ =
    data.plyometrics.length > 0
      ? Math.max(...data.plyometrics.map((p) => p.verticalJump))
      : null;
  const vjGoal = 30;
  ...
  if (loading)
    return (
      <main style={s.main}>
        <div style={s.loading}>CONNECTING...</div>
      </main>
    );
  ...
  return (
    <main style={s.main}>
      ...
      <div style={s.controlBar}>
        ...
        <button
          onClick={() => saveToCloud(data)}
          style={{ ...s.saveBtn, border: "1px solid #333" }}
        >
          FORCE SYNC
        </button>
      </div>
      ...
      <div style={s.statsBar}>
        <div style={s.stat}>
          <span style={s.statVal}>{data.streak}</span>
          <span style={s.statLabel}>STREAK</span>
        </div>
        <div style={s.stat}>
          <span style={s.statVal}>{weeklyCount}/5</span>
          <span style={s.statLabel}>WEEKLY</span>
        </div>
        <div style={s.stat}>
          <span style={{ ...s.statVal, color: "#FFD700" }}>{prCount}</span>
          <span style={s.statLabel}>PRs</span>
        </div>
      </div>
      ...
      <nav style={s.nav}>
        {["LOG", "HISTORY", "PLYO", "PRs"].map((k) => (
          <button
            key={k}
            onClick={() => setView(k.toLowerCase() as ViewMode)}
            style={{
              ...s.navBtn,
              borderBottom:
                view === k.toLowerCase() ? "2px solid #00FF7F" : "none",
              color: view === k.toLowerCase() ? "#00FF7F" : "#666",
            }}
          >
            {k}
          </button>
      ...
      <div style={s.content}>
          ...
          (currentWorkout ? (
            <div style={s.form}>
              <div style={s.formHeader}>
                <span>{currentWorkout.type.toUpperCase()}</span>
                <span style={{ color: "#666" }}>{currentWorkout.date}</span>
              </div>
              <div style={s.inputGroup}>
                ...
                {sets.length > 0 && (
                  <div style={s.setsList}>
                    {sets.map((st, i) => (
                      <div key={st.id} style={s.setItem}>
                        SET {i + 1}: {st.weight}×{st.reps} ={" "}
                        {st.weight * st.reps}
                      </div>
                    ))}
                  </div>
                ...
                {sets.length > 0 && (
                  <button onClick={saveExercise} style={s.exerciseBtn}>
                    SAVE EXERCISE
                  </button>
                )}
              </div>
                ...
                <div style={s.session}>
                  ...
                  {currentWorkout.exercises.map((ex, i) => (
                    <div key={i} style={s.exItem}>
                      {ex.name} {ex.isPR && <span style={s.prTag}>PR!</span>}{" "}
                      <span style={{ color: "#888" }}>
                        {ex.sets
                          .map((se) => `${se.weight}×${se.reps}`)
                          .join(", ")}
                      </span>
                    </div>
                  ))}
                  <div style={s.volume}>
                    VOL: {currentWorkout.totalVolume.toLocaleString()} lbs
                  </div>
                </div>
              )}
              <div style={s.actions}>
                <button onClick={cancelWorkout} style={s.cancelBtn}>
                  CANCEL
                </button>
                <button
                  ...
                  style={{
                    ...s.finishBtn,
                    opacity: currentWorkout.exercises.length === 0 ? 0.5 : 1,
                  }}
                >
                  FINISH
                </button>
            ...
            <div style={s.start}>
              ...
              <div style={s.typeGrid}>
                {(
                  ["push", "pull", "legs", "cardio", "plyo", "other"] as const
                ).map((t) => (
                  <button
                    key={t}
                    ...
                    style={{
                      ...s.typeBtn,
                      borderColor: workoutType === t ? "#00FF7F" : "#333",
                      color: workoutType === t ? "#00FF7F" : "#666",
                    }}
                  >
                    {t.toUpperCase()}
                  </button>
              ...
              <button onClick={startWorkout} style={s.startBtn}>
                START WORKOUT
              </button>
            </div>
          ))}
        ...
        {view === "history" && (
          <div style={s.historyList}>
            {data.workouts.length === 0 ? (
              <div style={s.empty}>No workouts yet.</div>
            ) : (
        ...
        {view === "plyo" && (
          <div style={s.plyo}>
            ...
            <div style={s.plyoForm}>
              <div style={s.plyoRow}>
                <div>
                  <label style={{ color: "#666", fontSize: "0.6rem" }}>
                    VERTICAL (in)
                  </label>
                  <input
                    type="number"
              ...
              <button onClick={logPlyo} style={s.logBtn}>
                LOG
              </button>
            </div>
            {data.plyometrics.length > 0 && (
        ...
        {view === "prs" && (
          <div style={s.prs}>
            {Object.keys(data.personalRecords).length === 0 ? (
              <div style={s.empty}>No PRs yet.</div>
            ) : (
              Object.entries(data.personalRecords)
                .sort(([, a], [, b]) => b - a)
                .map(([ex, w]) => (
                  <div key={ex} style={s.prItem}>
