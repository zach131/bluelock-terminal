...
const LOCAL_KEY = 'backup_threat_engine';

// ... (Keep Subject Map and Helpers: calculateGPAValue, getAvg, getSemesterAvg, calculateCumulativeGPA) ...
...
function calculateGPAValue(grade: number, level: 'L1' | 'L2'): number { return level === 'L1' ? (grade - 40) / 10 : (grade - 50) / 10; }
function getAvg(arr: GradeEntry[]): number { if (arr.length === 0) return -1; const sum = arr.reduce((a, b) => a + b.earned, 0); const poss = arr.reduce((a, b) => a + b.possible, 0); return poss === 0 ? -1 : (sum / poss) * 100; }
function getSemesterAvg(sem: SemesterData): number { const q1 = getAvg(sem.q1), q2 = getAvg(sem.q2), q3 = getAvg(sem.q3); if (q1 !== -1 && q2 !== -1) return (q1 + q2) / 2; if (q1 !== -1) return q1; if (q2 !== -1) return q2; if (q3 !== -1) return q3; return -1; }
function calculateCumulativeGPA(classes: ClassData[]): number { let totalPoints = 5.5, totalCredits = 1.0; classes.forEach(cls => { const s1 = getSemesterAvg(cls.sem1); if (s1 !== -1) { totalPoints += calculateGPAValue(s1, cls.level); totalCredits += 1.0; } const s2 = getSemesterAvg(cls.sem2); if (s2 !== -1) { totalPoints += calculateGPAValue(s2, cls.level); totalCredits += 1.0; } }); return totalCredits > 0 ? totalPoints / totalCredits : 0; }
function createDefaultClasses(): ClassData[] { return Object.values(SUBJECT_MAP).map(s => ({ id: s.id, name: s.name, level: s.level, sem1: { q1: [], q2: [], q3: [] }, sem2: { q1: [], q2: [], q3: [] } })); }
...
export default function ThreatEngine() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (json.success && json.data) { setData(json.data); setSaveStatus('saved'); localStorage.setItem(LOCAL_KEY, JSON.stringify(json.data)); addLog('ok', 'CLOUD LOADED'); }
        else { throw new Error("Empty"); }
      } catch (e) {
        const local = localStorage.getItem(LOCAL_KEY);
        if (local) { setData(JSON.parse(local)); setSaveStatus('local'); addLog('info', 'LOCAL RESTORE'); }
  ...
  // Persistence Layer
  const saveToCloud = useCallback(async (newData: ThreatData) => {
    setSaveStatus('saving');
    try {
      const res = await fetch('/api/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: SYNC_KEY, payload: newData }) });
      const json = await res.json();
      if(json.success) { setSaveStatus('saved'); localStorage.setItem(LOCAL_KEY, JSON.stringify(newData)); addLog('ok', 'SAVED'); setTimeout(() => setSaveStatus('syncing'), 2000); }
      else throw new Error("API");
    } catch (e) {
      setSaveStatus('error'); addLog('err', 'CONNECTION LOST'); localStorage.setItem(LOCAL_KEY, JSON.stringify(newData)); localStorage.setItem(`${SYNC_KEY}_pending`, 'true');
    }
  }, [addLog]);
  ...
  // ... (Keep Command Logic: handleCmd, injectGrades) ...
  useEffect(() => { const handleKey = () => { if(!importMode) inputRef.current?.focus(); }; window.addEventListener('keypress', handleKey); return () => window.removeEventListener('keypress', handleKey); }, [importMode]);
  ...
  const handleCmd = (raw: string) => {
    ...
    if (c.startsWith('+')) {
      const parts = raw.split(' '); const key = parts[0].substring(1); const period = parts[1]; const grades = parts.slice(2).map(g => parseInt(g)).filter(g => !isNaN(g));
  ...
  const injectGrades = (id: SubjectID, period: string, grades: number[]) => {
    setData(prev => {
      ...
      addLog('ok', `LOGGED: ${SUBJECT_MAP[id]?.name} [${period}]`);
      return { ...prev, classes: newClasses };
    });
  };

  const gpa = calculateCumulativeGPA(data.classes);
  const sortedClasses = [...data.classes].sort((a, b) => { const aAvg = getSemesterAvg(a.sem2) !== -1 ? getSemesterAvg(a.sem2) : getSemesterAvg(a.sem1); const bAvg = getSemesterAvg(b.sem2) !== -1 ? getSemesterAvg(b.sem2) : getSemesterAvg(b.sem1); return aAvg - bAvg; });
  ...
  return (
    <main style={s.main}>
      ...
      {importMode && ( <div style={s.overlay}> <div style={s.modal}> <div style={s.modalHeader}>DATA INJECTOR</div> <div style={s.modalBody}> <div style={s.row}> <select value={importData.subject} onChange={e => setImportData({...importData, subject: e.target.value})} style={s.select}> {Object.entries(SUBJECT_MAP).map(([k, v]) => <option key={k} value={k}>{v.name}</option>)} </select> <select value={importData.period} onChange={e => setImportData({...importData, period: e.target.value})} style={s.select}> <option value="q1">Q1</option><option value="q2">Q2</option><option value="q3">Q3</option> </select> </div> <textarea placeholder="Grades..." value={importData.grades} onChange={e => setImportData({...importData, grades: e.target.value})} style={s.textarea} /> <div style={s.modalActions}> <button onClick={() => setImportMode(false)} style={s.cancelBtn}>CANCEL</button> <button onClick={() => { injectGrades(importData.subject as SubjectID, importData.period, importData.grades.split(/[\s,]+/).map(Number).filter(n => !isNaN(n))); setImportMode(false); }} style={s.injectBtn}>INJECT</button> </div> </div> </div> </div> )}
      ...
      <header style={s.hud}>
        <div style={s.rowTop}>
          ...
          <div style={s.statusCont}>
             <span style={{ fontSize: '0.6rem', color: saveStatus === 'saved' ? '#00FF41' : saveStatus === 'saving' ? '#FFB000' : '#666' }}>
               {saveStatus === 'saved' ? '● SYNCED' : saveStatus === 'saving' ? '● SYNCING' : '○ DIRTY'}
             </span>
          </div>
        </div>
      ...
      {/* ... (Keep Class List JSX) ... */}
      <div style={s.list}>
        {sortedClasses.map(cls => { const s1 = getSemesterAvg(cls.sem1), s2 = getSemesterAvg(cls.sem2); return (
          <div key={cls.id} onClick={() => setExpanded(expanded === cls.id ? null : cls.id)} style={s.cardWrapper}>
            ...
            {expanded === cls.id && ( <div style={s.exp}> <div style={s.expSec}>Q1 [{cls.sem1.q1.length}]</div> <div style={s.gList}>{cls.sem1.q1.map((g,i) => <span key={i} style={s.gItem}>{g.earned}</span>)}</div> <div style={s.expSec}>Q2 [{cls.sem1.q2.length}]</div> <div style={s.gList}>{cls.sem1.q2.map((g,i) => <span key={i} style={s.gItem}>{g.earned}</span>)}</div> <div style={s.expSec}>Q3 [{cls.sem2.q3.length}]</div> <div style={s.gList}>{cls.sem2.q3.map((g,i) => <span key={i} style={s.gItem}>{g.earned}</span>)}</div> </div> )}
          </div>
        ); })}
      </div>
